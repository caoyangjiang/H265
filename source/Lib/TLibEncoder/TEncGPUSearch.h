...
namespace MemOpt
{
Int ChangeDevice(Int iDevice);

template <class T>
void AllocDeviceMem(T& src, size_t len);

template <class T>
void AllocHostMem(T& src, size_t len);

template <class T>
void TransferToDevice(T& src, T& targ, size_t len);

template <class T>
void TransferToHost(T& src, T& targ, size_t len);

template <class T>
void FreeDeviceMem(T* src);

template <class T>
void FreeHostMem(T* src);
}

namespace InterpolationKernel
{
void gpuGpuHalfSampleME(GpuMeDataAccess* pHostGMDA,
                        Int iStride,
                        Int iMvX,
                        Int iMvY);
}
...
/*
  //Debug
  m_puiSadArray[blockIdx.x * blockDim.x + threadIdx.x] = uiSad + (m_uiCost *
  uiTotalMvBits >> 16);
  //~Debug
...
/*
  //Debug
      ...
      GPU::MemOpt::TransferToHost<UInt*>(pHostGMDA->m_puiSadArray, tmp,
  sizeof(UInt)*iNumBlocks*iNumThreads);
...
/*
  //Not 100% correct
  ----------------------------------------------------------------------------------------------------------------------------------
  int iSharedMemorySize = pHostGMDA->m_iPUWidth * pHostGMDA->m_iPUHeight +
  2*(pHostGMDA->m_iSearchRange + pHostGMDA->m_iPUWidth -
  1)*pHostGMDA->m_iPUHeight;
  ----------------------------------------------------------------------------------------------------------------------------------
  ...
  for(y=0;y< iPUHeight;y++)
  {
    ...
    sPU[iPUHeight*iPUWidth + threadIdx.x + y * x ] = pYSR[threadIdx.x +
  y*iStride];

    if(threadIdx.x < iPUWidth)
    {
      sPU[iPUHeight*iPUWidth + threadIdx.x + 2*pDevGMDA->m_iSearchRange - 1 + y
  * x] = pYSR[2*pDevGMDA->m_iSearchRange - 1 + y*iStride];
    }
...
/*
  ...
  //#pragma unroll
    for(x=32;x>0;x>>=1)
    {
      if ((uiPU[threadIdx.x] & 0x00FFFFFF) < (uiPU[threadIdx.x + x] &
  0x00FFFFFF))
